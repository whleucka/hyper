...
final class QueryBuilderTest extends TestCase
{
  public function test_select_query(): void
  {
    ...
    $this->assertSame("SELECT id, email, name FROM users WHERE (id = ?) AND (name = ?) GROUP BY id, name HAVING (id = ?) AND (name = ?) ORDER BY id ASC, name DESC LIMIT 1, 2", $qb->build());
  }
  ...
  public function test_update_query(): void
  {
    ...
    $this->assertSame("UPDATE users SET name = ?, email = ? WHERE (id = ?)", $qb->build());
  }
